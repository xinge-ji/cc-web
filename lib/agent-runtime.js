...
function createAgentRuntime(deps) {
  ...
  function findPiSessionFile(session) {
    const sessionId = String(session?.piSessionId || '').trim();
    const sessionFile = String(session?.piSessionFile || '').trim();
    if (sessionFile && fs.existsSync(sessionFile)) return sessionFile;
    if (!sessionId) return '';
    const dir = session?.piSessionDir || (session?.id && piSessionDir ? piSessionDir(session.id) : '');
    ...
    while (stack.length > 0) {
      ...
      for (const entry of entries) {
        ...
        try {
          ...
          if (header.type === 'session' && header.id === sessionId) return fullPath;
        } catch {}
      }
    }
    return '';
  ...
  function buildPiSpawnSpec(session, options = {}) {
    const sessionDir = session?.piSessionDir || (session?.id && piSessionDir ? piSessionDir(session.id) : '');
    const sessionFile = findPiSessionFile(session);
    if (sessionFile) session.piSessionFile = sessionFile;
    if (sessionDir) session.piSessionDir = sessionDir;
  ...
  function piMessageKey(message) {
    return message?.id || message?.responseId || 'assistant';
  }

  function streamPiAssistantMessage(entry, message, sessionId) {
    if (!message || message.role !== 'assistant') return;
    const text = extractPiText(message.content);
    const key = piMessageKey(message);
    if (!entry.piMessageTextById) entry.piMessageTextById = new Map();
    const previous = entry.piMessageTextById.get(key) || '';
    if (text.length > previous.length) {
      const delta = text.slice(previous.length);
      appendFullText(entry, delta);
      wsSend(entry.ws, { type: 'text_delta', text: delta }, true);
    } else if (text !== previous && text && !entry.fullText.includes(text)) {
      appendFullText(entry, text);
      wsSend(entry.ws, { type: 'text_delta', text }, true);
    }
    entry.piMessageTextById.set(key, text);

    const usage = message.usage || null;
    if (usage) {
      if (!entry.piUsageByMessageId) entry.piUsageByMessageId = new Set();
      if (!entry.piUsageByMessageId.has(key)) {
        entry.piUsageByMessageId.add(key);
        const inputTokens = usage.input || 0;
        const cachedInputTokens = usage.cacheRead || 0;
        const outputTokens = usage.output || 0;
        const cost = Number(usage.cost?.total || 0);
        entry.lastUsage = { inputTokens, cachedInputTokens, outputTokens };
        entry.lastCost = cost || null;
        const session = loadSession(sessionId);
        if (session) {
          session.totalUsage = {
          ...
          if (cost) session.totalCost = (session.totalCost || 0) + cost;
          saveSession(session);
          wsSend(entry.ws, { type: 'usage', totalUsage: session.totalUsage }, true);
          if (cost) wsSend(entry.ws, { type: 'cost', costUsd: session.totalCost || 0 }, true);
        }
      }
    }

    if (message.stopReason === 'error' || message.stopReason === 'aborted') {
      entry.lastError = message.errorMessage || `Pi ${message.stopReason}`;
    }
  }

  function piToolMeta(toolName, args = {}, result = null) {
    if (toolName === 'bash') {
      const exitCode = typeof result?.details?.exitCode === 'number'
        ? result.details.exitCode
        : (typeof result?.exitCode === 'number' ? result.exitCode : null);
      return {
        kind: 'command_execution',
        title: 'Shell Command',
        subtitle: args?.command || '',
        exitCode,
        status: result ? 'completed' : 'in_progress',
      };
    }
    const target = args?.path || args?.pattern || args?.glob || args?.command || '';
    return {
      kind: 'mcp_tool_call',
      title: `Pi ${toolName || 'Tool'}`,
      subtitle: target,
      status: result ? 'completed' : 'in_progress',
    };
  }
  ...
  function processPiEvent(entry, event, sessionId) {
    ...
    switch (event.type) {
      case 'session': {
        ...
        const session = loadSession(sessionId);
        if (session) {
          setRuntimeSessionId(session, event.id);
          if (entry.piSessionDir) session.piSessionDir = entry.piSessionDir;
          const sessionFile = findPiSessionFile(session);
          if (sessionFile) session.piSessionFile = sessionFile;
          saveSession(session);
        }
      ...
      case 'message_update':
      case 'message_end':
        streamPiAssistantMessage(entry, event.message, sessionId);
        break;

      case 'tool_execution_start': {
        const id = event.toolCallId || `${event.toolName || 'pi-tool'}-${entry.toolCalls.length + 1}`;
        const tool = {
          name: event.toolName || 'PiTool',
          id,
          kind: event.toolName === 'bash' ? 'command_execution' : 'mcp_tool_call',
          meta: piToolMeta(event.toolName, event.args || {}),
          input: sanitizeToolInput(event.toolName || 'PiTool', event.args || {}),
          done: false,
        };
        if (!entry.toolCalls.find((item) => item.id === id)) {
          if (entry.toolCalls.length < MAX_TOOL_CALLS) entry.toolCalls.push(tool);
          else entry.toolCallsTruncated = true;
          wsSend(entry.ws, {
            type: 'tool_start',
            name: tool.name,
      ...
      case 'tool_execution_end': {
        ...
        const tc = entry.toolCalls.find((item) => item.id === id);
        if (!tc) break;
        tc.done = true;
        tc.meta = piToolMeta(event.toolName || tc.name, event.args || tc.input || {}, event.result || {});
        const resultText = piToolResultText(event.result).slice(0, 2000);
        tc.result = resultText;
        wsSend(entry.ws, {
          type: 'tool_end',
          toolUseId: id,
        ...
        if (event.isError && resultText) entry.lastError = resultText;
        break;
      }
      ...
      case 'extension_error':
        if (event.error) entry.lastError = String(event.error);
        break;
    }
  }
