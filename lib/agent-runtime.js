function createAgentRuntime(deps) {
  ...
  function buildCodexSpawnSpec(session, options = {}) {
    ...
    const permMode = session.permissionMode || 'yolo';
    const useAppServer = processEnv.CC_WEB_CODEX_TRANSPORT !== 'exec';
    if (useAppServer) {
      const appServerRuntimeId = session.codexThreadTransport === 'app-server' ? runtimeId : null;
      return {
        command: CODEX_PATH,
        args: [
          'app-server',
        ...
        mode: permMode,
        resume: !!appServerRuntimeId,
        model: session.model || '',
        runtimeId: appServerRuntimeId,
  ...
  function codexToolName(item) {
    const lowerName = String(item?.name || item?.tool || item?.tool_name || '').toLowerCase();
    if (item?.type === 'request_user_input' || lowerName === 'request_user_input') {
  ...
  function codexToolInput(item) {
    if (!item) return null;
    if (codexToolName(item) === 'AskUserQuestion') {
      if (Array.isArray(item.questions)) return { questions: item.questions };
      if (item.arguments && typeof item.arguments === 'object' && Array.isArray(item.arguments.questions)) return item.arguments;
      if (item.input && typeof item.input === 'object' && Array.isArray(item.input.questions)) return item.input;
      return { questions: [] };
    }
  ...
  function processCodexEvent(entry, event, sessionId) {
    ...
    switch (event.type) {
      case 'thread.started': {
        ...
        const session = loadSession(sessionId);
        if (session) {
          setRuntimeSessionId(session, event.thread_id, { codexThreadTransport: 'exec' });
      ...
      case 'turn.completed': {
        ...
        if (session && usage) {
          ...
          saveSession(session);
          wsSend(entry.ws, { type: 'usage', sessionId, totalUsage: session.totalUsage }, true);
        }
        break;
      }
